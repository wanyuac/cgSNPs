...
# Release: 11 Jan 2023; latest update: 11 Jan 2023
...
samtools merge -n -f --no-PG --output-fmt BAM -@ $t --reference $r -u -o ${s}_hybrid.bam ${s}_illumina_name_sorted.bam ${s}_nanopore_name_sorted.bam
samtools sort -@ $t --output-fmt BAM -o ${s}_hybrid_sorted.bam ${s}_hybrid.bam
samtools index -@ $t ${s}_hybrid_sorted.bam

# Clearance ===============
rm ${s}_illumina.sam ${s}_nanopore.sam ${s}_illumina_name_sorted.bam ${s}_nanopore_name_sorted.bam ${s}_hybrid.bam
